...
async function handleClickEditButton(bookId) {
  try {
    ...
    // TODO: answer here
    currentBook = await (await fetch(`http://localhost:3333/books/${bookId}`)).json();
...
async function handleClickDeleteButton(bookId) {
  try {
    ...
    // TODO: answer here
    // bener
    await deleteBook(bookId); //tambahkan await
...
async function handleEditForm(event) {
  try {
    ...
    */
    ...
    // TODO: answer here
    await editBook(book); //tambahkan await
...
async function handleAddForm(event) {
  try {
    ...
    */
    ...
    // TODO: answer here
    await addBook(book); //tambahkan await
...
function generateRows(books) {
  ...
  if (books.length === 0) {
    ...
    */
    ...
    rows = books.map(({ id, title, author, year, quantity }) => `
    <tr class="book-item">
...
async function loadPage() {
  switch (currentPage) {
    case 'home':
      ...
      */
      // TODO: answer here
      const rows = generateRows(books); tableBody.innerHTML = rows;
...
async function fetchBooks() {
  try {
    ...
    */
    // TODO: answer here
    const data = await (await fetch('http://localhost:3333/books')).json();
    return data;    
...
async function addBook(book) {
  try {
    ...
    */
    ...
    if (response.ok) console.log('Buku berhasil ditambahkan');
    else throw new Error('Buku gagal ditambahkan');
...
async function editBook(book) {
  try {
    ...
    */
    ...
    if (response.ok) console.log('Buku berhasil diubah');
    else throw new Error('Buku gagal diubah');
...
async function deleteBook(bookId) {
  try {
    ...
    */
    ...
    if (response.ok) console.log('Buku berhasil dihapus');
    else throw new Error('Buku gagal dihapus');
